...
class HttpdDaemon(HTTPServer):
        ...
        def do_GET(self):
            ...
            if urlparts.path in ('/boot', '/linux'):
                if 'mac' in query:
                    ...
                    for mac in macs:
                        mac = mac.upper().strip()
                        authorized = self.server.macs.get(mac, False)
                        log.info('MAC %s is %s' % \
                            (mac, authorized and 'authorized' or 'rejected'))
                        if authorized:
                            break
                else:
                    authorized = False
                    log.warn('Request does not specify a MAC')
                if authorized:
                    response = '\n\n' # HTTP protocol, line feed after headers
                    # dummy generation of a tester number
                    tester = sum([to_int('0x%s' %x) for x in mac.split('-')])
                    clientname = 'Tester-%03d' % (tester & 0xFF)
                    log.info("MAC %s is assigned as %s" % (mac, clientname))
                    response += 'Client: %s\n' % clientname
    ...
    def __init__(self, logger, parser):
        ...
        address = ('0.0.0.0', int(parser.get('httpd', 'port', '80')))
