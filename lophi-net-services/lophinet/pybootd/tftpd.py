...
from lophinet.pybootd import pybootd_path
...
#UI menu.c32
#MENU TITLE Boot
NETWORK_CONFIG = """DEFAULT clonezilla-auto-restore-physical
PROMPT 0

LABEL clonezilla-auto-restore-physical
        kernel clonezilla-live/vmlinuz
        append initrd=clonezilla-live/initrd.img boot=live config noswap nolocales edd=on nomodeset keyboard-layouts=none locales=en_US.UTF-8 vga=788 nosplash noprompt fetch=tftp://lophi-pxe/clonezilla-live/filesystem.squashfs ocs_prerun1="mount -t cifs //lophi-pxe/images/ /home/partimag -o user=guest,guest,noexec" ocs_live_run="ocs-sr --batch -e1 auto -e2 -r -j2 -p poweroff restoredisk %%image_name%% sda" ocs_live_extra_param=""
"""

def get_pxe_config_str(image_name):
    """
        Return our default PXE config with the image name substituted.
    """
    rtn = NETWORK_CONFIG.replace("%%image_name%%", image_name)
    return rtn
...
class TftpConnection(object):
    ...
    def parse(self, data, unpack=struct.unpack):
        ...
        if ( opcode == self.RRQ ) or ( opcode == self.WRQ ):
            ...
            # ADDED to deal with clonezilla path issues
            ...
            if self.server.root and self.is_url(self.server.root):
                ...
                try:
                    resource = os.path.join(self.server.root, resource)
    ...
    def handle_rrq(self, pkt):
        ...
        self.log.debug("Got rrq for %s"%resource)
        
        lophi_string = None
        
        # See if this is the clonezilla default image?
        rel_path = os.path.relpath(resource, self.server.root)
        if rel_path == "pxelinux.cfg/default":
            
            # Get our custom filename 
            client_ip = self.client_addr[0]
            custom_file = self.server.get_pxe_conf(client_ip)
            
            self.log.info("Checking custom LO-PHI request. (%s)"%custom_file)
            
            # Did LO-PHI define a custome filename?
            if custom_file is not None:
                
                # Define the file we are looking for
                custom_file_fullpath = os.path.join(os.path.dirname(resource), \
                                                    custom_file)
                
                images_dir = self.server.config.get('tftp','root_images',None)
                images = None
                if images_dir is not None:
                    images = os.listdir(images_dir)
                
                # Does this file exist?    
                if os.path.exists(custom_file_fullpath):
                    resource = custom_file_fullpath
                    self.log.info("Found resource file.")
                    
                # See if this directory exists in our images directory
                elif images is not None and custom_file in images:
                    lophi_string = get_pxe_config_str(custom_file)
                    self.log.info("Found image file.")
                    
                else:
                    self.log.error('%s does not exist.'%custom_file_fullpath)
                    
                 
            else:
                self.log.info('No special file here, returning default.')
            
#             lophi_type = 'clonezilla-live'
#             lophi_string = LOPHI_PXECONFIG[lophi_type]
        ...
        if 'tsize' in pkt and int(pkt['tsize']) == 0:
            # Custom LO-PHI file?
            if lophi_string is not None:
                filesize = len(lophi_string)
            elif genfile:
        ...
        # Sending a custom LO-PHI file?  
        if lophi_string is not None:
            self.log.info('Sending custom LO-PHI file.')
            self.file = StringIO(lophi_string)
        elif genfile:
...
class TftpServer(multiprocessing.Process):
    """TFTP Server
    Implements a threaded TFTP Server.
    Each request is handled in its own thread
    """

    def __init__(self, config_filename, bootpd=None,
                 mac_to_ip=None,mac_to_pxe_conf=None):
    ...
    def _get_mac(self,ip_address):
        """
            ...
            @return: MAC address given an IP
        """
        self.log.info("Resolving MAC for %s."%ip_address)
        if self.mac_to_ip is None:
            return None
        
        # Split IP to match the same format
        ip_address = ip_address.split(".")
        ip_address = map(int,ip_address)
    ...
    def get_pxe_conf(self,ip_address):
        """
            Given an IP look up the appropriate PXE config to serve up, then 
            remove it.
            
            @param ip_address: IP address to lookup
            @return: filename of PXE config 
        """
        self.log.info("Getting PXE config for %s."%ip_address)

        # Resolve our MAC first
        mac_addr = self._get_mac(ip_address)
        if mac_addr is None:
            return None
        
        if self.mac_to_pxe_conf is None:
            return None
        
        # Do we have a custom mapping?
        if mac_addr not in self.mac_to_pxe_conf.keys():
            return None
        else:
            rtn = self.mac_to_pxe_conf[mac_addr]
            return rtn 
    ...
    def bind(self):
        ...
        try:
            ...
            self.log.error("Could not bind to port. (%s:%d)"%(host,port))
